...
class users extends DbTable
{
	...
	public function __construct()
	{
		...
		// name
		$this->name = new DbField('users', 'users', 'x_name', 'name', '`name`', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// email
		$this->_email = new DbField('users', 'users', 'x__email', 'email', '`email`', '`email`', 200, -1, FALSE, '`email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_email->Sortable = TRUE; // Allow sort
		$this->fields['email'] = &$this->_email;

		// login
		$this->_login = new DbField('users', 'users', 'x__login', 'login', '`login`', '`login`', 200, -1, FALSE, '`login`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// user_level
		$this->user_level = new DbField('users', 'users', 'x_user_level', 'user_level', '`user_level`', '`user_level`', 3, -1, FALSE, '`user_level`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// profile_photo
		$this->profile_photo = new DbField('users', 'users', 'x_profile_photo', 'profile_photo', '`profile_photo`', '`profile_photo`', 200, -1, FALSE, '`profile_photo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->profile_photo->Sortable = TRUE; // Allow sort
		$this->fields['profile_photo'] = &$this->profile_photo;

		// profile_info
		$this->profile_info = new DbField('users', 'users', 'x_profile_info', 'profile_info', '`profile_info`', '`profile_info`', 201, -1, FALSE, '`profile_info`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->profile_info->Sortable = TRUE; // Allow sort
		$this->fields['profile_info'] = &$this->profile_info;
	}
	...
	{
		return ($this->SqlOrderBy <> "") ? $this->SqlOrderBy : "`user_level` ASC,`name` ASC";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->profile_photo->DbValue = $row['profile_photo'];
		$this->profile_info->DbValue = $row['profile_info'];
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->profile_photo->setDbValue($rs->fields('profile_photo'));
		$this->profile_info->setDbValue($rs->fields('profile_info'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->profile_photo->EditValue = $this->profile_photo->CurrentValue;
		...
		$this->profile_info->EditValue = $this->profile_info->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					if ($this->profile_photo->Exportable)
						$doc->exportCaption($this->profile_photo);
					if ($this->profile_info->Exportable)
						$doc->exportCaption($this->profile_info);
				} else {
					if ($this->id->Exportable)
						$doc->exportCaption($this->id);
					if ($this->name->Exportable)
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						if ($this->profile_photo->Exportable)
							$doc->exportField($this->profile_photo);
						if ($this->profile_info->Exportable)
							$doc->exportField($this->profile_info);
					} else {
						if ($this->id->Exportable)
							$doc->exportField($this->id);
						if ($this->name->Exportable)
