...
/**
 * Table class for pem_payment_type
 */
class pem_payment_type extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'pem_payment_type';
		$this->TableName = 'pem_payment_type';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`pem_payment_type`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// type_id
		$this->type_id = new DbField('pem_payment_type', 'pem_payment_type', 'x_type_id', 'type_id', '`type_id`', '`type_id`', 3, -1, FALSE, '`type_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// payment_type
		$this->payment_type = new DbField('pem_payment_type', 'pem_payment_type', 'x_payment_type', 'payment_type', '`payment_type`', '`payment_type`', 200, -1, FALSE, '`payment_type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// remarks
		$this->remarks = new DbField('pem_payment_type', 'pem_payment_type', 'x_remarks', 'remarks', '`remarks`', '`remarks`', 201, -1, FALSE, '`remarks`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`pem_payment_type`";
	}
	...
	{
		return ($this->SqlOrderBy <> "") ? $this->SqlOrderBy : "`payment_type` ASC";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->type_id->setDbValue($conn->insert_ID());
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "pem_payment_typelist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "pem_payment_typeview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "pem_payment_typeedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "pem_payment_typeadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("pem_payment_typeview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("pem_payment_typeview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "pem_payment_typeadd.php?" . $this->getUrlParm($parm);
		else
			$url = "pem_payment_typeadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("pem_payment_typeedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("pem_payment_typeadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("pem_payment_typedelete.php", $this->getUrlParm());
	}
	...
	public function renderEditRow()
	{
		...
		$this->payment_type->EditValue = $this->payment_type->CurrentValue;
